...
public class Implementation {
  ...
  /**
   * Returns the scope required to analyze the code to detect this issue. This is determined by the
   * detectors which reports the issue.
   *
   ...
   * For example, the unused resource detector needs to consider both the XML resource files and the
   * Java source files in order to decide if a resource is unused. If it analyzes just the Java
   * files for example, it might incorrectly conclude that a resource is unused because it did not
   ...
   * files and Java class files (detecting both layout constructor references in XML layout files as
   * well as code references in .class files). It doesn't have to analyze both; it is capable of
   * incrementally analyzing just an XML file, or just a class file, without considering the other.
   *
   * <p>The required scope list provides a list of scope sets that can be used to analyze this
   * issue. For each scope set, all the scopes must be matched by the incremental analysis, but any
   * one of the scope sets can be analyzed in isolation.
   *
   ...
   * by supplying it to {@link #Implementation(Class, java.util.EnumSet, java.util.EnumSet[])}. This
   * method returns true if the given scope matches one or more analysis scope, or the overall
   * scope.
   *
