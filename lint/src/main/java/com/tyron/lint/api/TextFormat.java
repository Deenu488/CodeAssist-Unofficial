...
/**
 * Lint error message, issue explanations and location descriptions are described in a {@link #RAW}
 * format which looks similar to text but which can contain bold, symbols and links. These issues
 * can also be converted to plain text and to HTML markup, using the {@link #convertTo(String,
 * TextFormat)} method.
 *
 * <p>// * @see Issue#getDescription(TextFormat)
 *
...
public enum TextFormat {
   ...
   *   <li>Sentences immediately surrounded by ` will be shown using monospace fonts
   * </ul>
   *
   * Furthermore, newlines are converted to br's when converting newlines. Note: It does not insert
   * {@code <html>} tags around the fragment for HTML output.
   *
   * <p>TODO: Consider switching to the restructured text format -
  ...
  private static void appendEscapedText(
      @NonNull StringBuilder sb, @NonNull String text, boolean html, int start, int end) {
    if (html) {
