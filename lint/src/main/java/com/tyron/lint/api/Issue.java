...
public final class Issue implements Comparable<Issue> {
  ...
  /**
   * Creates a new issue. The description strings can use some simple markup; see the {@link
   ...
   *     <b>problem</b> rather than the <b>fix</b> (e.g. "Missing minSdkVersion")
  ...
  public static Issue create(
      ...
      @NonNull Implementation mImplementation) {
    return new Issue(
        id, briefDescription, explanation, category, priority, severity, mImplementation);
  }
   ...
   * solved. Additional info can be provided via {@link #getMoreInfo()}.
   *
   ...
   *
   * <p>Note that even though the normal way for an issue to be disabled is for the {Configuration}
   * to return {@link Severity#IGNORE}, there is a {@link #isEnabledByDefault()} method which can be
   * used to turn off issues by default. This is done rather than just having the severity as the
   * only attribute on the issue such that an issue can be configured with an appropriate severity
   * (such as {@link Severity#ERROR}) even when issues are disabled by default for example because
   * they are experimental or not yet stable.
   *
  ...
  /**
   * Sets the implementation for the given issue. This is typically done by IDEs that can offer a
