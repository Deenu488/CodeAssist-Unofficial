...
public class Location {
  ...
  /**
   * (Private constructor, use one of the factory methods {@link Location#create(File)}, {@link
   * Location#create(File, Position, Position)}, or {@link Location#create(File, String, int, int)}.
   *
   * <p>Constructs a new location range for the given file, from start to end. If the length of the
   * range is not known, end may be null.
   *
   * @param file the associated file (but see the documentation for {@link #getFile()} for more
  ...
  /**
   * Returns the custom message for this location, if any. This is typically used for secondary
   * locations, to describe the significance of this alternate location. For example, for a
   * duplicate id warning, the primary location might say "This is a duplicate id", pointing to the
   * second occurrence of id declaration, and then the secondary location could point to the
   ...
   * by the creator of the {@link Location} to store temporary state associated with the location.
   *
   ...
   * the creator of the {@link Location} to store temporary state associated with the location.
   *
  ...
  @NonNull
  public static Location create(
      @NonNull File file, @NonNull Position start, @Nullable Position end) {
  ...
  @NonNull
  public static Location create(
      @NonNull File file, @Nullable String contents, int startOffset, int endOffset) {
    ...
    if (contents == null) {
      return new Location(
          file, new DefaultPosition(-1, -1, startOffset), new DefaultPosition(-1, -1, endOffset));
    }
   ...
   * @param contents the current contents of the file
   * @param line the line number (0-based) for the position
   * @param patternStart an optional pattern to search for from the line match; if found, adjust the
   *     column and offsets to begin at the pattern start
   * @param patternEnd an optional pattern to search for behind the start pattern; if found, adjust
   *     the end offset to match the end of the pattern
  ...
  public static Location create(
      ...
      int line,
      @Nullable String patternStart,
      @Nullable String patternEnd,
        ...
        if (index != -1) {
          ...
          if (patternEnd != null) {
            ...
            if (end != -1) {
              return new Location(
                  file,
                  new DefaultPosition(line, column, index),
            ...
            return new Location(
                ...
                new DefaultPosition(line, column, index),
                new DefaultPosition(
                    line, column + patternStart.length(), index + patternStart.length()));
          }
          return new Location(
              file,
              new DefaultPosition(line, column, index),
  ...
  private static int findPreviousMatch(
      @NonNull String contents, int offset, String pattern, @Nullable SearchHints hints) {
    while (true) {
  ...
  /**
   * A {@link Handle} is a reference to a location. The point of a location handle is to be able to
   * create them cheaply, and then resolve them into actual locations later (if needed). This makes
   * it possible to for example delay looking up line numbers, for locations that are offset based.
   */
  public interface Handle {
    ...
    /**
     * Sets the client data associated with this location. This is an optional field which can be
     * used by the creator of the {@link Location} to store temporary state associated with the
     * location.
     *
     * @param clientData the data to store with this location
     */
    ...
    /**
     * Returns the client data associated with this location - an optional field which can be used
     * by the creator of the {@link Location} to store temporary state associated with the location.
     *
     * @return the data associated with this location
     */
   ...
   *
   * <p>When dealing with bytecode for example, there are only line number entries within method
  ...
  /**
   * Extra information pertaining to finding a symbol in a source buffer, used by {@link
   * Location#create(File, String, int, String, String, SearchHints)}
   */
  public static class SearchHints {
    /**
     * the direction to search for the nearest match in (provided {@code patternStart} is non null)
     */
    @NonNull private final SearchDirection mDirection;
