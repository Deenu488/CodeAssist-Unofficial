...
public class CallSuperDetector extends Detector implements Detector.JavaScanner {
  ...
  private static final String CALL_SUPER_ANNOTATION =
      "androidx.annotation." + "CallSuper"; // $NON-NLS-1$
  ...
  private static final Implementation IMPLEMENTATION =
      new Implementation(CallSuperDetector.class, Scope.JAVA_FILE_SCOPE);
  ...
  private static void checkCallSuper(
      @NonNull JavaContext context, @NonNull MethodTree declaration) {
   ...
   */
  @Nullable
  private static ExecutableElement getRequiredSuperMethod(
      @NonNull JavaContext context, @NonNull MethodTree node) {
    ...
    List<? extends Element> elements =
        context.getCompileTask().task.getElements().getAllMembers(superElement);
  ...
  private static class SuperCallVisitor extends JavaVoidVisitor {
    ...
    public Void visitMethodInvocation(MethodInvocationTree methodInvocationTree, Void unused) {
      ...
      if (expressionTree instanceof MemberSelectTree) {
        boolean isThis =
            ((MemberSelectTree) expressionTree).getExpression().toString().equals("super");
        if (isThis) {
          if (mMethod
              .getSimpleName()
              .contentEquals(((MemberSelectTree) expressionTree).getIdentifier().toString())) {
            mCallsSuper = true;
            return null;
          }
        }
