...
public class ResourceNamespace implements Comparable<ResourceNamespace>, Serializable {
  ...
  public interface Resolver {
    ...
    /**
     * Contains a single mapping from "tools" to the tools URI. In the past we assumed the "tools:"
     * prefix is defined, we need to keep doing this for projects that don't care about namespaces.
     */
    Resolver TOOLS_ONLY =
        fromBiMap(ImmutableBiMap.of(SdkConstants.TOOLS_NS_NAME, SdkConstants.TOOLS_URI));

    /**
     * Creates a new {@link Resolver} which looks up prefix definitions in the given {@link BiMap}.
     *
  ...
  /**
   * Constructs a {@link ResourceNamespace} for the given (fully qualified) aapt package name. Note
   * that this is not the string used in XML notation before the colon (at least not in the general
   * case), which can be an alias.
   *
   * <p>This factory method can be used when reading the build system model or for testing, other
   * code most likely needs to resolve the short namespace prefix against XML namespaces defined in
   * the given context.
   *
  ...
  /**
   * Constructs a {@link ResourceNamespace} in code that does not keep track of namespaces yet, only
   * of the boolean `isFramework` flag.
   */
  @NotNull
  @Deprecated
  ...
  /**
   * Tries to build a {@link ResourceNamespace} from the first part of a {@link ResourceUrl}, given
   ...
   *     null), this is the namespace that will be returned. For example, if an XML file inside libA
   *     (com.lib.a) references "@string/foo", it means the "foo" resource from libA, so the
   *     "com.lib.a" namespace should be passed as the {@code defaultNamespace}.
   * @param resolver strategy for mapping short namespace prefixes to namespace URIs as used in XML
   *     resource files. This should be provided by the XML parser used. For example, if the source
   *     XML document contained snippet such as {@code
