...
public class ScopedException extends RuntimeException {
  ...
  public ScopedException(@Nullable Throwable cause, String message, Object... args) {
    super(
        message == null
            ? "unknown data binding exception"
  ...
  public String createHumanReadableMessage() {
    ...
    sb.append("ERROR: ")
        ...
        .append(" file://")
        .append(scopedError.getFilePath());
  ...
  private static ScopedException parseJson(String jsonError) throws JsonSyntaxException {
    ...
    List<Location> locations =
        encoded.positions.stream().map(FileLocation::toLocation).collect(Collectors.toList());

    return new ScopedException(
        encoded.message,
        new ScopedErrorReport(
            Strings.isNullOrEmpty(encoded.filePath) ? null : encoded.filePath, locations));
  }

  /**
   * Given encoded output generated by {@link #getMessage()}, extract out the list of {@link
   * ScopedException}s contained within.
   */
  public static List<ScopedException> extractErrors(String output) {
    Iterable<String> lines =
        Splitter.on(NEWLINE_PATTERN).omitEmptyStrings().trimResults().split(output);
