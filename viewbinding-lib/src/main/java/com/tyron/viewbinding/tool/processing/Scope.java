...
public class Scope {
  ...
  private static void registerErrorInternal(
      String msg, int scopeIndex, ScopeProvider... scopeProviders) {
   ...
   *
   * <p>This code actually starts entering the given scopes 1 by 1, starting from 0. When list is
   * consumed, it creates the exception and defers if possible then exits from the provided scopes.
   *
   * <p>Note that these scopes are added on top of the already existing scopes.
   *
  ...
  static String produceScopeLog() {
    ...
    while (top != null) {
      ...
      if (provider instanceof FileScopeProvider) {
        sb.append("file:")
            .append(((FileScopeProvider) provider).provideScopeFilePath())
            .append("\n");
      }
  ...
  private static List<Location> findAbsoluteLocationFrom(
      ScopeEntry entry, LocationScopeProvider top) {
  ...
  private static boolean validatedContained(ScopeEntry parent, Location absLocation) {
    ...
    List<Location> absoluteParents =
        findAbsoluteLocationFrom(parent, (LocationScopeProvider) provider);
