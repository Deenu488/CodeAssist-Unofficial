 ...
 *
 * <ul>
 *   <li>A plain 2-letter language descriptor
 *   <li>A 2-letter language descriptor followed by a -r 2 letter region qualifier
 *   <li>A plain 3-letter language descriptor
 *   <li>A 3-letter language descriptor followed by a -r 2 letter region qualifier
 *   <li>A BCP 47 language tag. The BCP-47 tag uses + instead of - as separators, and has the prefix
 *       b+. Therefore, the BCP-47 tag "zh-Hans-CN" would be written as "b+zh+Hans+CN" instead.
 * </ul>
 */
public final class LocaleQualifier extends ResourceQualifier {
  ...
  /**
   * Given a BCP-47 string, determines whether the string is already capitalized correctly (where
   * "correct" means for readability; all strings should be compared case insensitively)
   */
  @VisibleForTesting
  ...
  /**
   * Used only when constructing the qualifier, don't use after it's been assigned to a {@link
   * FolderConfiguration}.
   */
  void setRegionSegment(@NotNull String segment) {
    ...
    mRegion =
        new String(
            new char[] {
              Character.toUpperCase(segment.charAt(1)), Character.toUpperCase(segment.charAt(2))
            });
  ...
  /**
   * Parse an Android BCP-47 string (which differs from BCP-47 in that it has the prefix "b+" and
   * the separator character has been changed from - to +.
   *
   * @param qualifier the folder name to parse
   * @return a {@linkplain LocaleQualifier} holding the language, region and script or null if not a
   *     valid Android BCP 47 tag
   */
  @Nullable
