...
public class ElementsTrimmer {
   ...
   *
   * <p>Current trimmed elements are :
   *
   * <ul>
   *   <li>uses-features with glEsVersion key
   *       <ul>
   *         <li>The highest 1.x version element will be kept regardless of 'required' flag value
   *         <li>If the above element is present and has a 'false' required flag, there can be at
   *             most one element of a lesser version with 'required' attribute set to true.
   *         <li>The highest 2.x or superior element will be kept regardless of 'required' flag
   *             value
   *         <li>If the above element is present and has a 'false' required flag, there can be at
   *             most one element of a lesser version (but higher than 2.0) with a 'required'
  ...
  public static void trim(
    ...
    for (Map.Entry<Integer, XmlElement> glEsVersionDeclaration :
      ...
      boolean isRequired =
          requiredAttribute == null || Boolean.parseBoolean(requiredAttribute.getValue());

      if (glEsVersionDeclaration.getKey() < 0x20000) {
        // version one.
        removeElement =
            (doneWithBelowTwoFalse && doneWithBelowTwoTrue)
        ...
        removeElement =
            (doneWithAboveTwoFalse && doneWithAboveTwoTrue)
      ...
      if (removeElement) {
        ...
        if (glEsVersionDeclaration
          ...
          glEsVersionDeclaration
              .getValue()
              .getXml()
              .removeAttributeNS(ANDROID_URI, AndroidManifest.ATTRIBUTE_GLESVERSION);
          mergingReport
              .getActionRecorder()
              .recordAttributeAction(
                  glEsVersionDeclaration
                      .getValue()
                      .getAttribute(
                          XmlNode.fromXmlName("android:" + AndroidManifest.ATTRIBUTE_GLESVERSION))
                      .get(),
          ...
          xmlDocument
              .getRootNode()
              .getXml()
              .removeChild(glEsVersionDeclaration.getValue().getXml());
          mergingReport
              .getActionRecorder()
              .recordNodeAction(glEsVersionDeclaration.getValue(), Actions.ActionType.REJECTED);
        }
      }
    }
  }
  ...
  private static Integer getGlEsVersion(@NotNull XmlElement xmlElement) {
    Attr glEsVersion =
        xmlElement.getXml().getAttributeNodeNS(ANDROID_URI, AndroidManifest.ATTRIBUTE_GLESVERSION);
    if (glEsVersion == null) {
      return null;
    }
