 ...
 * an action by the merging tool.
 *
 * <p>This is primarily to catch situations like a user entered a tools:remove="foo" directory on
 * one of its elements and that particular attribute was never removed during the merges possibly
 * indicating an unforeseen change of configuration.
 *
 * <p>Most of the output from this validation should be warnings.
 */
public class PostValidator {
  ...
  public static void validate(
      @NotNull XmlDocument xmlDocument, @NotNull MergingReport.Builder mergingReport) {
    ...
    validate(xmlDocument.getRootNode(), mergingReport.getActionRecorder().build(), mergingReport);
    checkOnlyOneUsesSdk(xmlDocument, mergingReport);
  }

  /**
   * Enforces {@link SdkConstants#ANDROID_URI} declaration in the top level element. It is possible
   * that the original manifest file did not contain any attribute declaration, therefore not
   * requiring a xmlns: declaration. Yet the implicit elements handling may have added attributes
   * requiring the namespace declaration.
   */
  ...
  /**
   * Enforces {@link SdkConstants#TOOLS_URI} declaration in the top level element, if necessary. It
   ...
   *   <li>&lt;application&gt; is moved last in the list of children of the <manifest> element.
   *   <li>uses-sdk is moved first in the list of children of the &lt;manifest&gt; element
   * </ul>
   *
  ...
  private static void reOrderActivityAlias(@NotNull XmlElement xmlElement) {
    ...
    Optional<XmlElement> element =
        xmlElement.getNodeByTypeAndKey(ManifestModel.NodeTypes.APPLICATION, null);
    if (!element.isPresent()) {
      return;
    }
    XmlElement applicationElement = element.get();

    List<XmlElement> activityAliasElements =
    ...
    for (XmlElement activityAlias : activityAliasElements) {
      ...
      element =
          applicationElement.getNodeByTypeAndKey(ManifestModel.NodeTypes.ACTIVITY, targetActivity);
      if (!element.isPresent()) {
        continue;
      }
  ...
  private static void reOrderUsesSdk(@NotNull XmlElement xmlElement) {
    ...
    Optional<XmlElement> element =
        xmlElement.getNodeByTypeAndKey(ManifestModel.NodeTypes.USES_SDK, null);
    if (!element.isPresent()) {
      return;
    }
  ...
  private static void validate(
    ...
    switch (operationType) {
      case REPLACE:
        ...
        if (!ignoreWarning && !isNodeOperationPresent(xmlElement, actions, ActionType.REJECTED)) {
  ...
  /**
   * Check in our list of attribute actions that a particular {@link ActionType} action was recorded
   * on the passed element.
   *
   * @return true if it was applied, false otherwise.
   */
  private static boolean isAttributeOperationPresent(
    ...
    for (Actions.AttributeRecord attributeRecord :
        actions.getAttributeRecords(xmlElement.getId(), attributeOperation.getKey())) {
  ...
  private static void validateAndroidAttributes(
    ...
    for (XmlAttribute xmlAttribute : xmlElement.getAttributes()) {
      if (xmlAttribute.getModel() != null) {
        AttributeModel.Validator onWriteValidator = xmlAttribute.getModel().getOnWriteValidator();
