...
class MergerXmlUtils {
   ...
   * of schema. It is namespace aware.
   *
   * <p>This adds a user tag with the original {@link File} to the returned document. You can
   * retrieve this file later by using {@link #extractXmlFilename(Node)}.
   *
  ...
  static Document parseDocument(
      @NotNull final File xmlFile, @NotNull final IMergerLog log, @NotNull ManifestMerger merger) {
    try {
      ...
      log.error(
          Severity.ERROR, new FileAndLine(xmlFile.getAbsolutePath(), 0), "XML file not found");
  ...
  @Nullable
  static Document parseDocument(
      @NotNull String xml, @NotNull IMergerLog log, @NotNull FileAndLine errorContext) {
    try {
   ...
   * {@link #extractLineNumber(Node)}.
   *
   * <p>It also tries to add line number information, with the caveat that the current
   * implementation is a gross approximation.
   *
   * <p>There is no need to call this after calling one of the {@code parseDocument()} methods since
   * they already decorated their own document.
   *
   ...
   * information about whitespace between attributes.
   *
   * <p>Also we don't even try to deal with \n vs \r vs \r\n insanity. This only counts the \n
   * occurring in text nodes to determine line advances, which is clearly flawed.
   *
   * <p>However it's good enough for testing, and we'll replace it by a PositionXmlParser once it's
   * moved into com.android.util.
   */
   ...
   *
   * <p>TODO right now reformats the document. Needs to output as-is, respecting white-space.
   *
  ...
  static boolean printXmlFile(
      @NotNull Document doc, @NotNull File outFile, @NotNull IMergerLog log) {
   ...
   */
  static String printXmlString(@NotNull Document doc, @NotNull IMergerLog log) {
    try {
      Transformer tf = TransformerFactory.newInstance().newTransformer();
      tf.setOutputProperty(OutputKeys.OMIT_XML_DECLARATION, "yes"); // $NON-NLS-1$
      tf.setOutputProperty(OutputKeys.ENCODING, "UTF-8"); // $NON-NLS-1$
   ...
   *
   * <p>The map keys are "/manifest/elements...|attribute-ns-uri attribute-local-name", for example
  ...
  static void injectAttributes(
      @Nullable Document doc, @Nullable Map<String, String> attributeMap, @NotNull IMergerLog log) {
   ...
   *
   * <p>The output is designed to be printed using {@link #printXmlDiff}.
   *
   ...
   *     contains the expanded URIs but this is rather verbose so when printing the diff these will
   *     be replaced by the prefixes collected here.
  ...
  static String printElement(
      @NotNull Node node, @NotNull Map<String, String> nsPrefix, @NotNull String prefix) {
    StringBuilder sb = new StringBuilder();
   ...
   * #printElement(Node, Map, String)}.
   *
   * <p>If {@code nextSiblings} is false, the string conversion takes only the given child element
   * and stops there.
   *
   * <p>If {@code nextSiblings} is true, the string conversion also takes _all_ the siblings after
   * the given element. The idea is the caller can call this with the first child of a parent and
   * get a string showing all the children at the same time. They are sorted to avoid the ordering
   * issue.
   */
  @NotNull
   ...
   * String)}.
   *
   * <p>This is a <em>not</em> designed to be a full contextual diff. It just stops at the first
   * difference found, printing up to 3 lines of diff and backtracking to add prior contextual
   * information to understand the structure of the element where the first diff line occurred (by
   * printing each parent found till the root one as well as printing the attribute named by {@code
   * keyAttr}).
   *
   ...
   *     always insert the "name" attribute since that's the key element to help the user identify
   *     which node is being dumped.
   */
