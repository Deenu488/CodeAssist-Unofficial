 ...
 *
 * <p>Basically a facade object on {@link Attr} objects with some added features like automatic
...
public class XmlAttribute extends XmlNode {
  ...
  @Nullable private final AttributeModel mAttributeModel;
  ...
  void mergeInHigherPriorityElement(
      XmlElement higherPriorityElement, MergingReport.Builder mergingReport) {
    ...
    if (higherPriorityAttributeOptional.isPresent()) {
      ...
      handleBothAttributePresent(mergingReport, higherPriorityAttribute, attributeOperationType);
      return;
    }
    ...
    if (attributeOperationType == AttributeOperationType.REMOVE) {
      ...
      mergingReport
          .getActionRecorder()
          .recordAttributeAction(this, Actions.ActionType.REJECTED, AttributeOperationType.REMOVE);
      return;
    }
  ...
  private void handleBothAttributePresent(
    ...
    if (operationType == AttributeOperationType.REPLACE) {
      ...
      report
          .getActionRecorder()
          .recordAttributeAction(this, Actions.ActionType.REJECTED, AttributeOperationType.REPLACE);
      return;
    }
    // if the values are the same, then it's fine, otherwise flag the error.
    if (mAttributeModel != null) {
      String mergedValue =
          mAttributeModel.getMergingPolicy().merge(higherPriority.getValue(), getValue());
      if (mergedValue != null) {
   ...
   * not explicitly set on the implicitNode, yet it exist on this attribute {@link XmlElement}
  ...
  void mergeWithLowerPriorityDefaultValue(
    ...
    String mergedValue =
        mAttributeModel.getMergingPolicy().merge(getValue(), mAttributeModel.getDefaultValue());
    if (mergedValue == null) {
      addIllegalImplicitOverrideMessage(mergingReport, mAttributeModel, implicitNode);
    } else {
      ...
      mergingReport
          .getActionRecorder()
          .recordAttributeAction(
              this, Actions.ActionType.MERGED, null /* attributeOperationType */);
    }
  }
  ...
  private void addIllegalImplicitOverrideMessage(
    ...
    String error =
        String.format(
            "Attribute %1$s value=(%2$s) at %3$s"
  ...
  private void addConflictingValueMessage(
      MergingReport.Builder report, XmlAttribute higherPriority) {
  ...
  void addMessage(
      MergingReport.Builder report, MergingReport.Record.Severity severity, String message) {
