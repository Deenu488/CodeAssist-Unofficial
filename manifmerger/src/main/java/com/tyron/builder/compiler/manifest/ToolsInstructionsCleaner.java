 ...
 *
 * <p>All attributes belonging to the {@link SdkConstants#ANDROID_URI} namespace will be removed. If
 * an element contained a "tools:node=\"remove\"" attribute, the element will be deleted.
 */
@Immutable
public class ToolsInstructionsCleaner {
  ...
  public static XmlDocument cleanToolsReferences(
      @NotNull XmlDocument document, @NotNull ILogger logger) {

    document = Preconditions.checkNotNull(document);
    logger = Preconditions.checkNotNull(logger);
    MergingReport.Result result = cleanToolsReferences(document.getRootNode().getXml(), logger);
    return result == MergingReport.Result.SUCCESS ? document.reparse() : null;
  }

  private static MergingReport.Result cleanToolsReferences(Element element, ILogger logger) {
            ...
            if (element.getParentNode().getNodeType() == Node.DOCUMENT_NODE) {
              logger.error(
                  String.format(
                      ...
                      attribute.getNodeValue(), XmlNode.unwrapName(element)));
              return ERROR;
            } else {
