...
public class XmlElement extends OrphanXmlElement {
      ...
      if (SdkConstants.TOOLS_URI.equals(attribute.getNamespaceURI())) {
        ...
        if (instruction.equals(NodeOperationType.NODE_LOCAL_NAME)) {
          ...
          lastNodeOperationType =
              NodeOperationType.valueOf(SdkUtils.camelCaseToConstantName(attribute.getNodeValue()));
          ...
          try {
            attributeOperationType =
                AttributeOperationType.valueOf(SdkUtils.xmlNameToConstantName(instruction));
            ...
            try {
              ...
              String errorMessage =
                  String.format(
                      "Invalid instruction '%1$s', " + "valid instructions are : %2$s",
                      instruction, Joiner.on(',').join(AttributeOperationType.values()));
              throw new RuntimeException(
                  MergingException.wrapException(e1)
                      .withMessage(errorMessage)
                      .withFile(mDocument.getSourceFile())
                      .withPosition(mDocument.getNodePosition(xml))
                      .build());
            }
          }
          for (String attributeName :
              Splitter.on(',').trimResults().split(attribute.getNodeValue())) {
            if (attributeName.indexOf(XmlUtils.NS_SEPARATOR) == -1) {
              String toolsPrefix =
                  XmlUtils.lookupNamespacePrefix(
                      getXml(), SdkConstants.TOOLS_URI, SdkConstants.ANDROID_NS_NAME, false);
    ...
    for (int i = 0; i < namedNodeMap.getLength(); i++) {
      ...
      XmlAttribute xmlAttribute =
          new XmlAttribute(
              this,
              (Attr) attribute,
              getType().getAttributeModel(fromXmlName(((Attr) attribute).getName())));
   ...
   * com.google.common.base.Optional#absent} if not present.
   *
   ...
   * specify how conflicting elements should be handled, a {@link NodeOperationType#MERGE} will be
   * returned.
   */
  public NodeOperationType getOperationType() {
    return mNodeOperationType != null ? mNodeOperationType : NodeOperationType.MERGE;
  }
  ...
  public void mergeWithLowerPriorityNode(
      XmlElement lowerPriorityNode, MergingReport.Builder mergingReport) {
    ...
    if (mSelector != null && !mSelector.isResolvable(getDocument().getSelectors())) {
      mergingReport.addMessage(
          ...
          String.format(
              ...
              mSelector.toString(), Joiner.on(',').join(mDocument.getSelectors().getKeys())));
      return;
    }
    mergingReport
        .getLogger()
        .verbose("Merging " + getId() + " with lower " + lowerPriorityNode.printPosition());
    ...
    if (mNodeOperationType != NodeOperationType.MERGE_ONLY_ATTRIBUTES) {
      ...
      for (XmlElement lowerPriorityChild : lowerPriorityNode.getMergeableElements()) {
        mergingReport
            .getActionRecorder()
            .recordNodeAction(this, Actions.ActionType.REJECTED, lowerPriorityChild);
      }
    }
  }
  ...
  public Optional<XmlElement> getNodeByTypeAndKey(
    ...
    for (XmlElement xmlElement : mMergeableChildren) {
      if (xmlElement.isA(type) && (keyValue == null || keyValue.equals(xmlElement.getKey()))) {
  ...
  private void handleCustomElement(XmlElement customElement, MergingReport.Builder mergingReport) {
    ...
    String namespace =
        customElement
            ...
            .getRootNode()
            .getXml()
            .getAttribute(SdkConstants.XMLNS_PREFIX + prefix);

    if (namespace != null) {
      getDocument()
          .getRootNode()
          .getXml()
          .setAttributeNS(SdkConstants.XMLNS_URI, SdkConstants.XMLNS_PREFIX + prefix, namespace);
    }
  }
  ...
  private void mergeChildrenWithMultipleDeclarations(
      XmlElement lowerPriorityChild, MergingReport.Builder mergingReport) {
   ...
   *
   * <ul>
   *   <li>The associate {@link ManifestModel.NodeTypes} is annotated with {@link MergeType#IGNORE}
   *   <li>This element has a child of the same type with no key that has a ' tools:node="removeAll'
   *       attribute.
   * </ul>
   *
   * @param lowerPriorityChild the lower priority child we should determine eligibility for merging.
  ...
  private boolean shouldIgnore(XmlElement lowerPriorityChild, MergingReport.Builder mergingReport) {
    ...
    boolean shouldDelete =
        thisChild.mNodeOperationType == NodeOperationType.REMOVE_ALL
            && (thisChild.mSelector == null || thisChild.mSelector.appliesTo(lowerPriorityChild));
    // if we should discard this child element, record the action.
    if (shouldDelete) {
      mergingReport
          .getActionRecorder()
          .recordNodeAction(
              thisChildElementOptional.get(), Actions.ActionType.REJECTED, lowerPriorityChild);
    }
    return shouldDelete;
  }
  ...
  private void handleTwoElementsExistence(
      XmlElement higherPriority, XmlElement lowerPriority, MergingReport.Builder mergingReport) {
    ...
    switch (operationType) {
      ...
      case MERGE_ONLY_ATTRIBUTES:
        ...
        mergingReport
            .getActionRecorder()
            .recordNodeAction(higherPriority, Actions.ActionType.MERGED, lowerPriority);
      ...
      case REPLACE:
        ...
        mergingReport
            .getActionRecorder()
            .recordNodeAction(higherPriority, Actions.ActionType.REJECTED, lowerPriority);
        break;
      case STRICT:
  ...
  public Optional<String> compareTo(Object other) {
    ...
    if (getXml().getNamespaceURI() != null) {
      if (!getXml().getLocalName().equals(otherNode.getXml().getLocalName())) {
        return Optional.of(
            String.format(
                "Element names do not match: %1$s versus %2$s",
                getXml().getLocalName(), otherNode.getXml().getLocalName()));
      }
      // compare element ns
      ...
      if ((thisNS == null && otherNS != null) || (thisNS != null && !thisNS.equals(otherNS))) {
        return Optional.of(
            String.format(
                "Element namespaces names do not match: %1$s versus %2$s", thisNS, otherNS));
      }
    } else {
      if (!getXml().getNodeName().equals(otherNode.getXml().getNodeName())) {
        return Optional.of(
            String.format(
                "Element names do not match: %1$s versus %2$s",
                getXml().getNodeName(), otherNode.getXml().getNodeName()));
      }
    }
    ...
    if (expectedChildren.size() != actualChildren.size()) {
      ...
      if (expectedChildren.size() > actualChildren.size()) {
        // missing some.
        List<String> missingChildrenNames = Lists.transform(expectedChildren, NODE_TO_NAME);
  ...
  private Optional<String> findAndCompareNode(
      XmlElement otherElement, List<Node> otherElementChildren, XmlElement childNode) {
    ...
    return message.isPresent()
        ...
        : Optional.of(
            String.format(
                "Child %1$s not found in document %2$s",
                childNode.getId(), otherElement.printPosition()));
  }
    ...
    for (XmlAttribute expectedAttr : expected.getAttributes()) {
      ...
      if (actualAttr.isPresent()) {
        if (!expectedAttr.getValue().equals(actualAttr.get().getValue())) {
          return Optional.of(
              String.format(
                  "Attribute %1$s do not match: %2$s versus %3$s at %4$s",
                  expectedAttr.getId(),
        ...
        return Optional.of(
            String.format(
                "Attribute %1$s not found at %2$s", expectedAttr.getId(), actual.printPosition()));
      }
    }
    return Optional.absent();
  }
  ...
  void addMessage(
      MergingReport.Builder mergingReport, MergingReport.Record.Severity severity, String message) {
    mergingReport.addMessage(getSourceFilePosition(), severity, message);
  }
}
