...
public class JavaVisitor {
    ...
    for (Detector detector : detectors) {
      ...
      if (treeTypes != null) {
        for (Class<? extends Tree> tree : treeTypes) {
          List<VisitingDetector> list =
              mTreeTypeDetectors.computeIfAbsent(tree, k -> new ArrayList<>(SAME_TYPE_COUNT));
          list.add(v);
        }
      }
      ...
      if (names != null) {
        for (String name : names) {
          List<VisitingDetector> list =
              mMethodDetectors.computeIfAbsent(name, k -> new ArrayList<>(SAME_TYPE_COUNT));
          list.add(v);
        }
      }
    }
   ...
   * specialized form of the {@link DispatchVisitor}.
   */
