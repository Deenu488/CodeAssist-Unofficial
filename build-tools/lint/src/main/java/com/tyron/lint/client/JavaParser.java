...
public abstract class JavaParser {
  ...
  /**
   * Prepare to parse the given contexts. This method will be called before a series of {@link
   * #parseJava(JavaContext)} calls, which allows some parsers to do up front global computation in
   * case they want to more efficiently process multiple files at the same time. This allows a
   * single type-attribution pass for example, which is a lot more efficient than performing global
   * type analysis over and over again for each individual file
   *
   ...
   *     Context#getContents()} but the file handle ( {@link Context#file} can also be used to map
   *     to an existing editor buffer in the surrounding tool, etc)
  ...
  public Location getLocation(@NonNull JavaContext context, @NonNull PsiElement element) {
    ...
    return Location.create(
        context.file, context.getContents(), range.getStartOffset(), range.getEndOffset());
  }

  /**
   * Returns a {@link Location} for the given node range (from the starting offset of the first node
   * to the ending offset of the second node).
   *
  ...
  public Location getRangeLocation(
    ...
    int end =
        Math.min(
            contents == null ? Integer.MAX_VALUE : contents.length(),
            to.getTextRange().getEndOffset() + toDelta);
    return Location.create(context.file, contents, start, end);
  }

  /**
   * Returns a {@link Location} for the given node. This attempts to pick a shorter location range
   * than the entire node; for a class or method for example, it picks the name node (if found). For
   * statement constructs such as a {@code switch} statement it will highlight the keyword, etc.
   *
   * @param context information about the file being parsed
  ...
  /**
   * Dispose any remaining data structures held for all contexts. Typically frees up any resources
   * allocated by {@link #prepareJavaParse(List)}
   */
  public void dispose() {}
}
