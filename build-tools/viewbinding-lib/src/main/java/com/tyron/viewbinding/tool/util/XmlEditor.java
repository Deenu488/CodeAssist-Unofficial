...
public class XmlEditor {
  ...
  public static String strip(File f, String newTag, String encoding) throws IOException {
    ...
    List<? extends ElementContext> layoutNodes = excludeNodesByName("data", childrenOfRoot);
    if (layoutNodes.size() != 1) {
      L.e(
          "Only one layout element with 1 view child is allowed. %s has %d",
    ...
    if (noTagRoot != null) {
      TagAndContext newRootTag =
          new TagAndContext(noTagRoot.getTag() + rootAttributes.toString(), layoutNode);
      int index = noTag.indexOf(noTagRoot);
      noTag.set(index, newRootTag);
    } else {
      TagAndContext newRootTag = new TagAndContext(rootAttributes.toString(), layoutNode);
      noTag.add(newRootTag);
    }
    //noinspection NullableProblems
    Collections.sort(
        noTag,
        new Comparator<TagAndContext>() {
          @Override
    ...
    for (TagAndContext it : noTag) {
      ...
      String newLine =
          line.substring(0, endTagPosition.charIndex)
              + " "
              + tag
  ...
  private static <T extends XMLParser.ElementContext> List<T> filterNodesByName(
      String name, Iterable<T> items) {
  ...
  private static <T extends XMLParser.ElementContext> List<T> excludeNodesByName(
      String name, Iterable<T> items) {
    List<T> result = new ArrayList<T>();
    for (T item : items) {
      if (!name.equals(nodeName(item))) {
  ...
  public static List<? extends AttributeContext> attributes(ElementContext elementContext) {
    if (elementContext.attribute() == null) return new ArrayList<AttributeContext>();
    else {
  ...
  private static boolean replace(
      ArrayList<String> lines, Position start, Position end, String text) {
  ...
  private static int recurseReplace(
      ElementContext node,
  ...
  private static PositionPair findTerminalPositions(ElementContext node, ArrayList<String> lines) {
