 ...
 *
 * <p>TODO: more work necessary, this is pretty raw as it stands.
 */
@Immutable
public class MergingReport {
  ...
  /**
   * Returns all the merging intermediary stages if {@link
   * ManifestMerger2.Invoker.Feature#KEEP_INTERMEDIARY_STAGES} is set.
   */
   ...
   *
   * <p>Once the merging is finished, the {@link #build()} is called to return an immutable version
  ...
  static class Builder {
    ...
    Builder addMessage(
        ...
        @NotNull String message) {
      return addMessage(
          new SourceFilePosition(sourceFile, SourcePosition.UNKNOWN), severity, message);
    }
    ...
    MergingReport build() {
      Result result = mHasErrors ? Result.ERROR : mHasWarnings ? Result.WARNING : Result.SUCCESS;
