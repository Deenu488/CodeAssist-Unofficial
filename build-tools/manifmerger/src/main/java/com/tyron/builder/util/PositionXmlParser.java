...
public class PositionXmlParser {
   ...
   * Parses the XML content from the given input stream and closes the stream.
   *
   * <p>If a non-recoverable parser error is encountered, parsing stops, an error message is added
   * to the {@code parseErrors} list, and the returned document contains the elements up to the one
  ...
  private static Node findNodeAtLineAndCol(@NotNull Node node, int line, int column) {
    ...
    if (p != null) {
      ...
      if (end != null) {
        if (line > end.getLine()
            || line == end.getLine() && column != -1 && column >= end.getColumn()) {
          return null;
        }
      }
    } else {
  ...
  private static final class DomBuilder extends DefaultHandler2 {
    ...
    /**
     * Adds a node to the current parent element being visited, or to the document if there is no
     * parent in context.
     *
    ...
    /**
     * Find opening tags from the current position. '<' cannot appear in attribute values or
     * anywhere else within an element open tag, so we know the first occurrence is the real element
     * start. For comments, it is not legal to put '<' in a comment, however we are not validating
     * so we will return an invalid column in that case.
     *
