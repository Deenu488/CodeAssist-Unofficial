...
public interface ResourceEnum {
  ...
  /**
   * Whether the value actually used on device. This returns true only if a device can report this
   * value, false if it's just used to qualify resources.
   */
  boolean isValidValueForDevice();

  /**
   * Whether the value is neither used for device nor resources. This returns false when the value
   * is only used for internal usage in the custom editors.
   */
  boolean isFakeValue();
  ...
  /**
   * Returns a long string for display value. This must not only include the enum value but context
   * (qualifier) about what the value represents.
   *
   * <p>For instance "Exposed keyboard", and "Export navigation", as "exposed" would not be enough
   * to know what qualifier the value is about.
   *
