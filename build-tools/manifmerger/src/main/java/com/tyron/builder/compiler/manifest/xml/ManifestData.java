...
public final class ManifestData {
  ...
  /**
   * Value returned by {@link #getMinSdkVersion()} when the value of the minSdkVersion attribute in
   * the manifest is a codename and not an integer value.
   */
  public static final int MIN_SDK_CODENAME = 0;

  /**
   * Value returned by {@link #getGlEsVersion()} when there are no {@code <uses-feature>} node with
   * the attribute glEsVersion set.
   */
  ...
  /**
   * API level requirement. Default is 1 even if missing. If value is a codename, then it'll be 0
   * instead.
   */
  ...
  public static final class Activity {
    ...
    /**
     * If the activity doesn't yet have a filter set for the launcher, this resets both flags. This
     * is to handle multiple intent-filters where one could have the valid action, and another one
     * of the valid category.
     */
  ...
  /**
   * Class representing the <code>supports-screens</code> node in the manifest. By default, all the
   * getters will return null if there was no value defined in the manifest.
   *
   * <p>To get an instance with all the actual values, use {@link
   * #resolveSupportsScreensValues(int)}
   */
  public static final class SupportsScreens {
    ...
    /**
     * Instantiate an instance from a string. The string must have been created with {@link
     * #getEncodedValues()}.
     *
    ...
    /**
     * Returns an instance of {@link SupportsScreens} initialized with the default values based on
     * the given targetSdkVersion.
     *
    ...
    public static SupportsScreens getDefaultValues(int targetSdkVersion) {
      ...
      result.mResizeable =
          result.mAnyDensity =
              result.mSmallScreens =
                  result.mLargeScreens = targetSdkVersion <= 3 ? Boolean.FALSE : Boolean.TRUE;

      return result;
    }

    /**
     * Returns a version of the receiver for which all values have been set, even if they were not
     * present in the manifest.
     *
     * @param targetSdkVersion the target api level of the app, since this has an effect on default
     *     values.
     */
    ...
    /**
     * Returns either {@link Boolean#TRUE} or {@link Boolean#FALSE} based on the value of the given
     * Boolean object.
     */
    ...
    /**
     * Returns true if the two instances support the same screen sizes. This is similar to {@link
     * #equals(Object)} except that it ignores the values of {@link #getAnyDensity()} and {@link
     * #getResizeable()}.
     *
    ...
    /**
     * Returns true if the two instances have strictly different screen size support. This means
     * that there is no screen size that they both support.
     *
     * @param support the other instance to compare to.
     ...
     * anyDensity), and considers that {@link #hasStrictlyDifferentScreenSupportAs(SupportsScreens)}
     * returns true and {@link #overlapWith(SupportsScreens)} returns false.
     *
     * @throws IllegalArgumentException if the two instanced are not strictly different or overlap
     *     each other
     ...
     * instantiate a {@link SupportsScreens} object through {@link #SupportsScreens(String)}.
     */
    ...
    /**
     * Returns true if the two instance overlap with each other. This can happen if one instances
     * supports a size, when the other instance doesn't while supporting a size above and a size
     * below.
     *
    ...
    public boolean overlapWith(SupportsScreens otherSS) {
      if (mSmallScreens == null
          || mNormalScreens == null
          || mLargeScreens == null
          || otherSS.mSmallScreens == null
          || otherSS.mNormalScreens == null
          || otherSS.mLargeScreens == null) {
        throw new IllegalArgumentException("Some screen sizes Boolean are not initialized");
      }

      if (mSmallScreens == Boolean.TRUE
          && mNormalScreens == Boolean.FALSE
          && mLargeScreens == Boolean.TRUE) {
        return otherSS.mNormalScreens == Boolean.TRUE;
      }

      if (otherSS.mSmallScreens == Boolean.TRUE
          && otherSS.mNormalScreens == Boolean.FALSE
  ...
  /**
   * Returns the <code>targetSdkVersion</code> attribute, or the same value as {@link
  ...
  /**
   * Returns the glEsVersion from a {@code <uses-feature>} or {@link #GL_ES_VERSION_NOT_SET} if not
   * set.
   */
  public int getGlEsVersion() {
  ...
  /**
   * Returns the {@link SupportsScreens} object representing the <code>supports-screens</code> node,
   * or null if the node doesn't exist at all. Some values in the {@link SupportsScreens} instance
   * maybe null, indicating that they were not present in the manifest. To get an instance that
   * contains the values, as seen by the Android platform when the app is running, use {@link
  ...
  /**
   * Returns an always non-null instance of {@link SupportsScreens} that's been initialized with the
   * default values, and the values from the manifest. The default values depends on the manifest
   * values for minSdkVersion and targetSdkVersion.
   */
  public synchronized SupportsScreens getSupportsScreensValues() {
    if (mSupportsScreensValues == null) {
      if (mSupportsScreensFromManifest == null) {
        ...
        mSupportsScreensValues =
            mSupportsScreensFromManifest.resolveSupportsScreensValues(getTargetSdkVersion());
      }
    }
