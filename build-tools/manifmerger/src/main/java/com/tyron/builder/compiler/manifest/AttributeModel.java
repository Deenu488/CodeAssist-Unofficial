...
class AttributeModel {
   ...
   *
   * @param name name of the attribute, so far assumed to be in the {@link SdkConstants#ANDROID_URI}
   *     namespace.
  ...
  /**
   * Returns the attribute's {@link AttributeModel.Validator} to validate its value when read from
   * xml files or null if no validation is necessary.
   */
  @Nullable
  ...
  /**
   * Returns the attribute's {@link AttributeModel.Validator} to validate its value when the merged
   * file is about to be persisted.
   */
  @Nullable
  ...
  /** Returns the {@link AttributeModel.MergingPolicy} for this attribute. */
  @NotNull
  ...
  static class Builder {
    ...
    /**
     * Sets a {@link AttributeModel.Validator} to validate the attribute's values coming from xml
     * files.
     */
    ...
    /**
     * Sets a {@link AttributeModel.Validator} to validate values before they are written to the
  ...
  static final MergingPolicy STRICT_MERGING_POLICY =
      new MergingPolicy() {
  ...
  static final MergingPolicy OR_MERGING_POLICY =
      new MergingPolicy() {
        @Override
        public boolean shouldMergeDefaultValues() {
          return true;
        ...
        public String merge(@NotNull String higherPriority, @NotNull String lowerPriority) {
          return Boolean.toString(
              BooleanValidator.isTrue(higherPriority) || BooleanValidator.isTrue(lowerPriority));
        }
      };
   ...
   * value
   */
  static final MergingPolicy NO_MERGING_POLICY =
      new MergingPolicy() {
  ...
  private static int decodeDecOrHexString(String s) {
    long decodedValue =
        s.startsWith("0x") || s.startsWith("0X") ? Long.decode(s) : Long.parseLong(s);
   ...
   *
   * <p>The validator can be called when xml documents are read to ensure the xml file contains
   * valid statements.
   *
   * <p>This is a poor-mans replacement for not having a proper XML Schema do perform such
   * validations.
   */
  interface Validator {
  ...
  static class BooleanValidator implements Validator {
    ...
    public boolean validates(
        ...
        @NotNull String value) {
      boolean matches =
          TRUE_PATTERN.matcher(value).matches() || FALSE_PATTERN.matcher(value).matches();
      if (!matches) {
  ...
  /**
   * A {@link AttributeModel.Validator} for verifying that a proposed value is part of the
   * acceptable list of possible values.
   */
  ...
  /**
   * A {@link AttributeModel.Validator} for verifying that a proposed value is a numerical integer
   * value.
   */
  ...
  /**
   * A {@link AttributeModel.Validator} to validate that a string is a valid 32 bits hexadecimal
   * representation.
   */
    ...
    public boolean validates(
      ...
      if (!valid) {
        attribute.addMessage(
            ...
            MergingReport.Record.Severity.ERROR,
            String.format(
                "Attribute %1$s at %2$s is not a valid hexadecimal 32 bit value," + " found %3$s",
                attribute.getId(), attribute.printPosition(), value));
      }
      return valid;
    }
  ...
  /**
   * A {@link AttributeModel.Validator} to validate that a string is a valid 32 positive hexadecimal
   * representation with a minimum value requirement.
   */
