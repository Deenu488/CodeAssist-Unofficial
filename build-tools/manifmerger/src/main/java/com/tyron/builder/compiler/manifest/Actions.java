...
public class Actions {
  ...
  // TODO: i18n
  @VisibleForTesting static final String HEADER = "-- Merging decision tree log ---\n";
   ...
   */
  @NotNull
  public ImmutableList<AttributeRecord> getAttributeRecords(
      XmlNode.NodeKey elementKey, XmlNode.NodeName attributeName) {
  ...
  public abstract static class Record {
    ...
    public void print(StringBuilder stringBuilder) {
      stringBuilder.append(mActionType).append(" from ").append(mActionLocation);
      if (mReason != null) {
  ...
  private static class NodeNameDeserializer implements JsonDeserializer<XmlNode.NodeName> {
    ...
    @Override
    public XmlNode.NodeName deserialize(
    ...
    if (decisionTreeRecord != null) {
      ...
      for (XmlAttribute xmlAttribute : element.getAttributes()) {
        Actions.AttributeRecord attributeRecord =
            findAttributeRecord(decisionTreeRecord, xmlAttribute);
  ...
  private static Actions.AttributeRecord findAttributeRecord(
      DecisionTreeRecord decisionTreeRecord, XmlAttribute xmlAttribute) {
    for (Actions.AttributeRecord attributeRecord :
        decisionTreeRecord.getAttributeRecords(xmlAttribute.getName())) {
  ...
  /**
   * Internal structure on how {@link Actions.Record}s are kept for an xml element.
   *
   * <p>Each xml element should have an associated DecisionTreeRecord which keeps a list of {@link
   * Actions.NodeRecord} for all the node actions related to this xml element.
   *
   * <p>It will also contain a map indexed by attribute name on all the attribute actions related to
