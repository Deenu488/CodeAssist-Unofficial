...
public class XmlDocument {
  ...
  private final PlaceholderHandler.KeyBasedValueResolver<ManifestMerger2.SystemProperty>
  ...
  public XmlDocument(
      ...
      @NotNull
          PlaceholderHandler.KeyBasedValueResolver<ManifestMerger2.SystemProperty>
   ...
   * @return a new merged {@link XmlDocument} or {@link Optional#absent()} if there were errors
   *     during the merging activities.
   */
  public Optional<XmlDocument> merge(
      XmlDocument lowerPriorityDocument, MergingReport.Builder mergingReportBuilder) {
    ...
    getRootNode()
        .mergeWithLowerPriorityNode(lowerPriorityDocument.getRootNode(), mergingReportBuilder);
  ...
  /**
   * Returns the {@link PlaceholderHandler.KeyBasedValueResolver} capable of resolving all injected
   * {@link ManifestMerger2.SystemProperty}
   */
  public PlaceholderHandler.KeyBasedValueResolver<ManifestMerger2.SystemProperty>
      getSystemPropertyResolver() {
    return mSystemPropertyResolver;
  }

  /**
   * Compares this document to another {@link XmlDocument} ignoring all attributes belonging to the
  ...
  public Optional<XmlAttribute> getPackage() {
    ...
    return packageAttribute.isPresent()
        ? packageAttribute
        : getRootNode()
            .getAttribute(XmlNode.fromNSName(SdkConstants.ANDROID_URI, "android", "package"));
  }
  ...
  private String getRawMinSdkVersion() {
    ...
    if (usesSdk.isPresent()) {
      Optional<XmlAttribute> minSdkVersion =
          usesSdk.get().getAttribute(XmlNode.fromXmlName("android:minSdkVersion"));
  ...
  private String getMinSdkVersion() {
    ...
    String injectedMinSdk =
        mSystemPropertyResolver.getValue(ManifestMerger2.SystemProperty.MIN_SDK_VERSION);
  ...
  private String getRawTargetSdkVersion() {
    ...
    if (usesSdk.isPresent()) {
      Optional<XmlAttribute> targetSdkVersion =
          usesSdk.get().getAttribute(XmlNode.fromXmlName("android:targetSdkVersion"));
  ...
  private String getTargetSdkVersion() {
    ...
    String injectedTargetVersion =
        mSystemPropertyResolver.getValue(ManifestMerger2.SystemProperty.TARGET_SDK_VERSION);
  ...
  private void addImplicitElements(
      XmlDocument lowerPriorityDocument, MergingReport.Builder mergingReport) {
    ...
    if (usesSdk.isPresent()) {
      ...
      if (usesSdkElement.getOperationType() != NodeOperationType.MERGE) {
        mergingReport.addMessage(
            new SourceFilePosition(getSourceFile(), usesSdkElement.getPosition()),
    ...
    boolean hasWriteToExternalStoragePermission =
        lowerPriorityDocument
            .getByTypeAndKey(
                ManifestModel.NodeTypes.USES_PERMISSION, permission("WRITE_EXTERNAL_STORAGE"))
            .isPresent();
    ...
    if (thisTargetSdk >= 16 && libraryTargetSdk < 16) {
      if (lowerPriorityDocument
          .getByTypeAndKey(ManifestModel.NodeTypes.USES_PERMISSION, permission("READ_CONTACTS"))
          .isPresent()) {
        addIfAbsent(
            mergingReport.getActionRecorder(),
            ManifestModel.NodeTypes.USES_PERMISSION,
      ...
      if (lowerPriorityDocument
          .getByTypeAndKey(ManifestModel.NodeTypes.USES_PERMISSION, permission("WRITE_CONTACTS"))
          .isPresent()) {
        addIfAbsent(
            mergingReport.getActionRecorder(),
            ManifestModel.NodeTypes.USES_PERMISSION,
  ...
  private boolean checkUsesSdkMinVersion(
    ...
    int libraryMinSdk = getApiLevelFromAttribute(lowerPriorityDocument.getRawMinSdkVersion());
  ...
  private Optional<Element> addIfAbsent(
    ...
    Element elementNS =
        getXml().createElementNS(SdkConstants.ANDROID_URI, "android:" + nodeType.toXmlName());
    ...
    if (attributes != null) {
      for (Pair<String, String> attribute : attributes) {
        elementNS.setAttributeNS(
            SdkConstants.ANDROID_URI, "android:" + attribute.first, attribute.second);
      }
    }
