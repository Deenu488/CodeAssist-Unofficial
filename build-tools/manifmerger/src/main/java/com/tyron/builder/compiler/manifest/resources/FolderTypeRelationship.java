...
/**
 * This class gives access to the bidirectional relationship between {@link ResourceType} and {@link
 * ResourceFolderType}.
 */
public final class FolderTypeRelationship {
  ...
  /**
   * Returns a list of {@link ResourceType}s that can be generated from files inside a folder of the
   * specified type.
   *
   ...
   */
  @NotNull
  public static List<ResourceType> getRelatedResourceTypes(@NotNull ResourceFolderType folderType) {
  ...
  /**
   * Returns a single ResourceType that can be generated from files inside a folder of the specified
   * type and which is not {@link ResourceType#ID}.
   *
   * @param folderType The folder type.
  ...
  public static ResourceType getNonIdRelatedResourceType(@NotNull ResourceFolderType folderType) {
    ...
    if (resourceTypes.isEmpty()) {
      throw new IllegalArgumentException(
          ...
          // should never happen
          String.format(
              "No resource types defined for given folder type: %s", folderType.getName()));
    }
  ...
  /**
   * Returns a list of {@link ResourceFolderType} that can contain files generating resources of the
   * specified type.
   *
  ...
  /**
   * Returns the {@link ResourceFolderType} corresponding to a given {@link ResourceType}, that is a
   * folder that can contain files generating resources of the specified type and is not {@link
   ...
   * @param folderType the folder type.
   * @return true if files inside the folder of the specified {@link ResourceFolderType} could
   *     generate a resource of the specified {@link ResourceType}
   */
  public static boolean match(
      @NotNull ResourceType resType, @NotNull ResourceFolderType folderType) {
    List<ResourceFolderType> list = mTypeToFolderMap.get(resType);
  ...
  /**
   * Adds a {@link ResourceType} - {@link ResourceFolderType} relationship. this indicates that a
   * file in the folder can generate a resource of the specified type.
   *
  ...
  private static void makeSafe() {
    for (ResourceType type : ResourceType.values()) {
      ...
      if (list != null) {
        // getNonValuesRelatedFolder above assumes every resource belongs in only one non-values
        // folder.
