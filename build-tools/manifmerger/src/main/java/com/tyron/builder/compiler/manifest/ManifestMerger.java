 ...
 *
 * <p>To use, create with {@link ManifestMerger#ManifestMerger(IMergerLog, ICallback)} then call
 * {@link ManifestMerger#process(File, File, File[], Map, String)}.
 *
 * <p>
 *
...
public class ManifestMerger {
   ...
   *
   * <p>When true, the merged document is revisited and class names attributes are shortened when
   * possible, e.g. the package prefix is removed from the class name if it matches.
   *
   ...
   *
   * <p>This does NOT stop on errors, in an attempt to accumulate as much info as possible to return
   * to the user. Unless it failed to read the main manifest, a result file will be created. However
   * if process() returns false, the file should not be used except for debugging purposes.
   *
   ...
   *     "/manifest/uses-sdk|http://schemas.android.com/apk/res/android minSdkVersion". (note the
   *     space separator between the attribute URI and its local name.) The elements will be created
   *     if they don't exists. Existing attributes will be modified. The replacement is done on the
   *     main document <em>before</em> merging.
  ...
  public boolean process(
    ...
    if (mainDoc == null) {
      mLog.error(
          Severity.ERROR,
          new FileAndLine(mainFile.getAbsolutePath(), 0),
    ...
    if (!MergerXmlUtils.printXmlFile(mainDoc, outputFile, mLog)) {
      mLog.error(
          Severity.ERROR,
          new FileAndLine(outputFile.getAbsolutePath(), 0),
   ...
   * to the user.
   *
   * <p>The method might modify the input XML document in-place for its own processing.
   *
  ...
  public boolean process(
    ...
    if (packageOverride != null) {
      MergerXmlUtils.injectAttributes(
          mainDoc, Collections.singletonMap("/manifest| package", packageOverride), mLog);
    }
   ...
   * to the user.
   *
   * <p>The method might modify the input XML documents in-place for its own processing.
   *
   * @param mainDoc The document to merge into. Will be modified in-place.
  ...
  private boolean mergeLibDoc(Document libDoc) {
    ...
    boolean skipApplication =
        hasOverrideOrRemoveTag(findFirstElement(mMainDoc, "/manifest/application")); // $NON-NLS-1$
   ...
   *
   * <p>Some manifest attributes represent class names. These can be specified as fully qualified
   * class names or use a short notation consisting of just the terminal class simple name or a dot
   * followed by a partial class name. Unfortunately this makes textual comparison of the attributes
   * impossible. To simplify this, we can modify the document to fully expand all these class names.
   * The list of elements and attributes to process is listed by {@link #sClassAttributes} and the
   * expansion simply consists of appending the manifest' package if defined.
   *
  ...
  private void expandFqcns(Document doc) {
    ...
    if (pkg == null || pkg.isEmpty()) {
      ...
      mLog.error(
          Severity.WARNING, xmlFileAndLine(manifest), "Missing 'package' attribute in manifest.");
      return;
    }
      ...
      for (int i = 0; i < elements.getLength(); i++) {
        ...
        if (elem instanceof Element) {
          ...
          if (attr != null) {
            ...
            // or does not contain any dot.
            if (value != null
                && value.length() > pkgLength
                && value.startsWith(pkg)
                && value.charAt(pkgLength) == '.') {
   ...
   * all present and equal in the destination and prints a warning if it's not the case.
   *
   * <p>For example if a library supports a given screen configuration, print a warning if the main
   ...
   * uses a library which does. On the other hand we don't want to silently ignore this fact.
   *
   * <p>TODO there should be a way to silence this warning. The current behavior is certainly
   * arbitrary and needs to be tweaked somehow.
   *
   ...
   * be identical.
   *
   * <p>When merging, append to the end of the application element. Also merges any preceding
   * whitespace and up to one comment just prior to the merged element.
   *
   * @param path The XPath of the elements to merge from the library. Must not be null.
  ...
  private boolean mergeNewOrEqual(String path, String keyAttr, Document libDoc, boolean warnDups) {
    ...
    if (parent == null) {
      mLog.error(
          Severity.ERROR, xmlFileAndLine(mMainDoc), "Could not find element %1$s.", parentPath);
      return false;
    }
   ...
   * out-variables must be preset to their default before the call.
   *
   * <p>destValue/srcValue will be filled with the integer value of the field, if present and a
   * correct number, in which case destImplied/destImplied are also set to true. Otherwise the
   * values and the implied variables are left untouched.
   */
   ...
   * comment nodes and their preceding whitespace.
   *
   * <p>This may returns either {@code end} or a previous sibling. Never returns null.
   */
  @NotNull
   ...
   * element.
   *
   * <p>Implementation detail: this clones the source nodes into the destination.
   *
   ...
   */
  @Nullable
  private Element findFirstElement(@NotNull Document doc, @NotNull String path) {
    Node result;
    try {
   ...
   */
  private List<Element> findElements(@NotNull Document doc, @NotNull String path) {
   ...
   * Finds zero or more elements matching the given XPath expression in the given document.
   *
   * <p>Furthermore, the elements must have an attribute matching the given attribute name and value
   * if provided. (If you don't need to match an attribute, use the other version.)
   *
   * <p>Note that if you provide {@code attrName} as non-null then the {@code attrValue} must be
   * non-null too. In this case the XPath expression will be modified to add the check by naively
   ...
   * from which the XML node was parsed.
   *
   * <p>When the line number is unknown (e.g. if a {@link Document} instance is given) then line
   ...
   *
   * <p>If an element is marked with tools:merge=override, this attribute is removed. If an element
   * is marked with tools:merge=remove, the <em>whole</em> element is removed.
   *
  ...
  private static int insertSourceMarker(
      @NotNull Node parent, @NotNull Node node, @NotNull File file, boolean after) {
    int insertCount = 0;
    Document doc =
